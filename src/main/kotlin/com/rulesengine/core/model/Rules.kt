    ...
    companion object {
        private val mapModelRules: Map<String, Rule<Model, AttackTarget?>> = mapOf(
                Pair("Disgustingly Resilient", Rule("Disgustingly Resilient",
                        ...
                        { model: Model, options: List<Option>, attackTarget: AttackTarget? -> model.keywords.any { keyword -> keyword.toLowerCase() == "Death guard".toLowerCase() } },
                        { model: Model ->
                            val copy = model.copy()
                ...
                Pair("Combi-weapon used", Rule("Combi-weapon",
                        ...
                        { model: Model, options: List<Option>, attackTarget: AttackTarget? -> options.contains(Option.BothGunsCombi) },
                        { model: Model ->
                            val copy = model.copy()
                ...
                Pair("Rites of Battle", Rule("Rites of Battle",
                        ...
                        { model: Model, options: List<Option>, attackTarget: AttackTarget? -> true },
                        { model: Model ->
                            val copy = model.copy()
        ...
        fun findModelRule(ruleName: String): Rule<Model, AttackTarget?> {
            return mapModelRules[ruleName]
                    ?: Rule("Empty", "", { model: Model, options: List<Option>, attackTarget: AttackTarget? -> true }, { model: Model -> model })
        }
        ...
        private val mapWeaponRules: Map<String, Rule<Weapon, AttackTarget?>> = mapOf(
                Pair("Supercharged", Rule("Supercharged",
                        ...
                        { weapon: Weapon,options: List<Option>, attackTarget: AttackTarget? -> weapon.name == "Plasma incinerator" && options.contains(Option.ToHeat) },
                        { weapon: Weapon ->
                ...
                Pair("Slow", Rule("Slow",
                        ...
                        { weapon: Weapon, options: List<Option>, attackTarget: AttackTarget? -> true },
                        { weapon: Weapon ->
                            val copy = weapon.copy()
