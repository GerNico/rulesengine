    ...
    companion object {
        private val mapModelRules: Map<String, Rule<Model, AttackTarget?>> = mapOf(
                Pair("Disgustingly Resilient", Rule("Disgustingly Resilient",
                        ...
                        { model: Model, option: Option, attackTarget: AttackTarget? -> model.keywords.any { keyword -> keyword.toLowerCase() == "Death guard".toLowerCase() } },
                        { model: Model ->
                            val copy = model.copy()
        ...
        fun findModelRule(ruleName: String): Rule<Model, AttackTarget?> {
            return mapModelRules[ruleName]
                    ?: Rule("Empty", "", { model: Model, option: Option, attackTarget: AttackTarget? -> true }, { model: Model -> model })
        }
        ...
        private val mapWeaponRules: Map<String, Rule<Weapon, AttackTarget?>> = mapOf(
                Pair("Supercharged", Rule("Supercharged",
                        ...
                        { weapon: Weapon, option: Option, attackTarget: AttackTarget? -> weapon.name == "Plasma incinerator" && option == Option.ToHeat },
                        { weapon: Weapon ->
                ...
                Pair("Slow", Rule("Slow",
                        ...
                        { weapon: Weapon, option: Option, attackTarget: AttackTarget? -> true },
                        { weapon: Weapon ->
                            val copy = weapon.copy()
